      ...
      if (dates.length > 0) {
        ...
        if (futureDate) {
          ...
          if (daysUntil >= 3 && daysUntil <= 90) {
            ...
            const officialLinkRegex = /https?:\/\/(?:www\.)?(callofduty\.com)[\w\/\-\.]+/g;
...
async function scrapeOfficialBlog(): Promise<WipeData | null> {
  const page = await newPage();
  try {
    console.log("🔍 Checking Call of Duty Official Blog...");
    await page.goto("https://www.callofduty.com/blog", {
    ...
    const articleData = await page.evaluate(() => {
      ...
      for (const article of articles) {
        ...
        if (lowerTitle.includes("season") && (lowerTitle.includes("announcement") || lowerTitle.includes("release"))) {
    ...
    if (articleData) {
      ...
      if (potentialDate > new Date()) {
        potentialDate.setUTCHours(16, 0, 0, 0); // CoD seasons start at 9 AM PT
        ...
        return {
          ...
          lastWipe: lastSeason.toISOString(),
          frequency: "Every ~2 months (60-70 days)",
          source: "Call of Duty Blog (Official)",
          scrapedAt: new Date().toISOString(),
          confirmed: true,
          announcement: title,
          patchNotes: link,
    ...
    console.error("❌ Error scraping CoD Blog:", error);
    return null;
  } finally {
    await page.close();
  }
...
async function scrapeFandom(): Promise<WipeData | null> {
  // Official blog is now the primary source, Fandom is legacy.
  return null;
}
