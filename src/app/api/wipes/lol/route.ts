...
import { scrapeLoLSeasons } from '@/lib/scrapers/lol-reddit';
import { readFileSync, writeFileSync, existsSync, mkdirSync } from 'fs';
import { join } from 'path';
import type { WipeData } from '@/schemas/wipe-data';
import { getCacheControlHeader, CachePresets } from '@/lib/cache-headers';

const CACHE_FILE = join(process.cwd(), 'cache', 'lol-wipe.json');
...
async function getCachedData(forceRefresh: boolean) {
  ...
  try {
    ...
    console.log('Scraping fresh LoL season data...');
    const data = await scrapeLoLSeasons();
    writeCache(data);
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch League of Legends data' },
      {
        status: 500,
        headers: {
