...
import { scrapeFortniteWipe } from '@/lib/scrapers/fortnite';
import { readFileSync, writeFileSync, existsSync, mkdirSync } from 'fs';
import { join } from 'path';
import type { WipeData } from '@/schemas/wipe-data';
import { getCacheControlHeader, CachePresets } from '@/lib/cache-headers';

const CACHE_FILE = join(process.cwd(), 'cache', 'fortnite-wipe.json');
...
async function getCachedWipeData(forceRefresh: boolean) {
  ...
  try {
    ...
    console.log('Scraping fresh Fortnite data...');
    const data = await scrapeFortniteWipe();
    writeCache(data);
