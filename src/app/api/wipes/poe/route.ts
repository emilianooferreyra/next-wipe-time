...
import { scrapePoeWipe } from "@/lib/scrapers/poe";
import {
  validateCachedData,
...
const CACHE_FILE = join(process.cwd(), "cache", "poe-wipe.json");
...
function writeCache(data: WipeData | undefined) {
  try {
    if (!data) {
      console.warn("⚠️ Attempted to cache undefined data");
      return;
    }
    ensureCacheDir();
...
async function getCachedWipeData(forceRefresh: boolean) {
  ...
  try {
    ...
    console.log("Scraping fresh PoE data from official sources...");
    const data = await scrapePoeWipe();
    writeCache(data);
