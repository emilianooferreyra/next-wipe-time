...
import { scrapeCODSeasons } from '@/lib/scrapers/cod-fandom';
import { readFileSync, writeFileSync, existsSync, mkdirSync } from 'fs';
import { join } from 'path';
import type { WipeData } from '@/schemas/wipe-data';
import { getCacheControlHeader, CachePresets } from '@/lib/cache-headers';

const CACHE_FILE = join(process.cwd(), 'cache', 'cod-wipe.json');
...
async function getCachedData(forceRefresh: boolean) {
  ...
  try {
    ...
    console.log('Scraping fresh Call of Duty season data...');
    const data = await scrapeCODSeasons();
    writeCache(data);
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch Call of Duty data' },
      {
        status: 500,
        headers: {
