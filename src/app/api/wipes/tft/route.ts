...
import { scrapeTFTSets } from '@/lib/scrapers/tft-reddit';
import { readFileSync, writeFileSync, existsSync, mkdirSync } from 'fs';
import { join } from 'path';
import type { WipeData } from '@/schemas/wipe-data';
import { getCacheControlHeader, CachePresets } from '@/lib/cache-headers';

const CACHE_FILE = join(process.cwd(), 'cache', 'tft-wipe.json');
...
async function getCachedData(forceRefresh: boolean) {
  ...
  try {
    ...
    console.log('Scraping fresh TFT set data...');
    const data = await scrapeTFTSets();
    writeCache(data);
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch TFT data' },
      {
        status: 500,
        headers: {
