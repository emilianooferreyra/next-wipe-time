...
const CACHE_FILE = join(process.cwd(), "cache", "apex-wipe.json");
...
async function getCachedData(forceRefresh: boolean) {
  ...
  try {
    ...
    console.log("Scraping fresh Apex Legends season data...");
    const data = await scrapeApexSeasons();
    writeCache(data);
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Failed to fetch Apex Legends data" },
      {
        status: 500,
        headers: {
