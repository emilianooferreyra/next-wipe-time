...
import { scrapeDBDChapters } from '@/lib/scrapers/dbd-fandom';
import { readFileSync, writeFileSync, existsSync, mkdirSync } from 'fs';
import { join } from 'path';
import type { WipeData } from '@/schemas/wipe-data';
import { getCacheControlHeader, CachePresets } from '@/lib/cache-headers';

const CACHE_FILE = join(process.cwd(), 'cache', 'dbd-wipe.json');
...
async function getCachedData(forceRefresh: boolean) {
  ...
  try {
    ...
    console.log('Scraping fresh Dead by Daylight chapter data...');
    const data = await scrapeDBDChapters();
    writeCache(data);
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch Dead by Daylight data' },
      {
        status: 500,
        headers: {
