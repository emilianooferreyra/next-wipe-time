...
import { scrapeR6SiegeSeason } from '@/lib/scrapers/r6siege';
import { readFileSync, writeFileSync, existsSync, mkdirSync } from 'fs';
import { join } from 'path';
import type { WipeData } from '@/schemas/wipe-data';
import { getCacheControlHeader, CachePresets } from '@/lib/cache-headers';

const CACHE_FILE = join(process.cwd(), 'cache', 'r6siege-season.json');
...
async function getCachedSeasonData(forceRefresh: boolean) {
  ...
  try {
    ...
    console.log('Scraping fresh Rainbow Six Siege season data...');
    const data = await scrapeR6SiegeSeason();
    writeCache(data);
